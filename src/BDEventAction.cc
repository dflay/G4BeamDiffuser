...
void BDEventAction::EndOfEventAction(const G4Event* event)
{
  ...
  if(fICHCID == -1) fICHCID = G4SDManager::GetSDMpointer()->GetCollectionID("IonChamberHitsCollection");

  // Get hits collections
  // auto absoHC = GetHitsCollection(fAbsHCID, event);
  // auto gapHC  = GetHitsCollection(fGapHCID, event);
  // auto diffHC = GetHitsCollection(fDiffHCID, event);
  auto icHC = GetHitsCollection(fICHCID, event);

  int NHits = icHC->entries();
  std::cout << "[BDEventAction::EndOfEventAction]: Event " << eventID << ", hits = " << NHits << std::endl;
  if(NHits==0){
     return;
  }
  ...
  auto icHit   = (*icHC)[icHC->entries()-1];   // THIS IS THE LAST HIT! 
  ...
  G4double EDEP = icHit->GetEdep()/CLHEP::GeV; 
  G4double ETOT = icHit->GetTotalEnergy()/CLHEP::GeV; 
  G4double TLEN = icHit->GetTrackLength()/CLHEP::mm;

  G4ThreeVector pos = icHit->GetPos();
  G4double x = pos.getX()/CLHEP::mm; 
  G4double y = pos.getY()/CLHEP::mm; 
  G4double z = pos.getZ()/CLHEP::mm;

  G4ThreeVector labPos = icHit->GetLabPos();
  G4double lx = labPos.getX()/CLHEP::mm; 
  G4double ly = labPos.getY()/CLHEP::mm; 
  G4double lz = labPos.getZ()/CLHEP::mm;

  G4ThreeVector mom = icHit->GetMomentum();
